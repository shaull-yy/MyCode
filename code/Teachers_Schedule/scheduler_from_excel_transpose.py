...
INPUT_FILE = 'file:///C:/_Shaul/Projects/_My_Code/code/Teachers_Schedule/scheduling_inputs.xlsx'
OUTPUT_FILE = 'file:///C:/_Shaul/Projects/_My_Code/code/Teachers_Schedule/class_schedule.xlsx'
...
def export_to_excel(schedule, filename=OUTPUT_FILE):
	writer = pd.ExcelWriter(filename, engine='xlsxwriter')

	# Determine structure from first class
	...
	for c, slots in schedule.items():
		...
		rows = []
		for h in hours:
			row = [slots.get((d, h), '---') for d in days]
			rows.append(row)

		df = pd.DataFrame(rows, index=[f"Hour {h}" for h in hours], columns=days)
		df.to_excel(writer, sheet_name=c)

	writer.close()
	print(f"✅ Transposed schedule exported to: {filename}")
